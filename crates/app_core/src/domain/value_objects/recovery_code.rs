...
impl RecoveryCode {
    ...
    /// check symbol before returning `Ok`. A wrong length or an unknown symbol
    /// is `MalformedFormat`; a well-formed code with a mismatched check symbol
    /// is `InvalidChecksum` and fails fast — well before any AES-GCM unwrap
    /// attempt in `infra_credentials`.
    pub fn parse(input: &str) -> Result<Self, RecoveryCodeError> {
        let normalized: String = input
            ...
            .collect();
        if normalized.len() != Self::TOTAL_LEN {
            return Err(RecoveryCodeError::MalformedFormat);
        }
        if !normalized[..Self::DATA_LEN].chars().all(is_crockford_char) {
            return Err(RecoveryCodeError::MalformedFormat);
        }
        let data_sum: u32 = normalized[..Self::DATA_LEN]
            .chars()
            .map(crockford_value)
            .sum();
        ...
        if normalized.as_bytes()[Self::DATA_LEN] != expected_check as u8 {

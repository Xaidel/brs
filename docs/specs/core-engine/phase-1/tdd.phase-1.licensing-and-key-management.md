...
This document does **not** define: RBAC/Staff Accounts enforcement, Resident/Household/Certificate domain modeling, the Audit Trail's own entity, the automated backup engine's scheduling/retention logic, Barangay Identity/Officials/Templates/Purok/Theming/Dashboard, any UI/Tauri command wiring beyond what §9 requires implementers to know exists, or `infra_*` implementation code. Those are Phase 2 and Phase 3, scoped but not designed here (§5, §13).
...
- The automated Backup & Recovery engine — scheduling, 14-snapshot rolling retention, manual export (§9.8) beyond the Recovery-Code key-recovery path this document does cover — Phase 3.
...
This document's slice touches `app_core`, `infra_hardware_id`, `infra_licensing`, `infra_credentials`, `infra_persistence` (for `LicenseGrantRepository` only), and `src-tauri` (for the `Clock` adapter placement, §4.5). It does not touch `infra_backup`.
...
| §9.4 Certificate Generation & Control Number Sequencing | Phase 2 | Control Number Format token grammar remains an open question (§15, inherited from PRD §14). |
...
| §9.8 Backup & Recovery | Phase 1 (Recovery-Code key recovery only) / Phase 3 (scheduling, retention, manual export) | This document's `RecoverDatabaseEncryptionKey` use case (§7.4) covers "restore using Recovery Code, no original hardware" for the *key*; the snapshot engine itself (daily schedule, 14-snapshot rolling window, manual USB export) is Phase 3 and lives in `infra_backup`, out of this document's crate touch-list (§4.1). |
| §9.9 Barangay Identity & Branding | Phase 3 | — |
| §9.10 Barangay Officials & Signatures | Phase 3 | — |
| §9.11 Certificate Template & Layout Customization | Phase 3 | Template Variable enumeration remains an open question (§15, inherited from PRD §14). |
...
| §11 data model: `BackupSnapshot` | Phase 3 | `bootstrap.json`'s presence inside every Backup Snapshot's file set is a Phase 1 constraint on Phase 3's snapshot engine (§9), not designed here. |
...
First-run only. Calls `EncryptionCredentialGateway::establish()` (§8.4), which internally generates `system_secret`, salt, and a Recovery Code, persists the wrapped bootstrap state and the OS-keyring copy of `system_secret`, and returns the `RecoveryCode` value. The use case returns that `RecoveryCode` for one-time display — it is never persisted in plaintext anywhere `app_core` can see, and this use case does not log it.
...
- `src-tauri`: implements `SystemClock`; at startup, calls `EncryptionCredentialGateway::load_database_key()` (or, on the first-run path, `establish()`) directly as composition-root plumbing (§7.6) before wiring `infra_persistence`'s SQLCipher connection; exposes Tauri commands that call `GetMachineHardwareIdUseCase`, `ActivateLicenseUseCase`, `GetActiveLicenseUseCase`, `EstablishEncryptionCredentialUseCase`, and `RecoverDatabaseEncryptionKeyUseCase` through `app_core`'s public assembly API.
...
- **`bootstrap.json`** (backing `EncryptionCredentialGateway`, owned by `infra_credentials`, not a SQL table): salt (raw bytes, base64 in JSON) and the AES-256-GCM-wrapped `system_secret` (nonce + ciphertext + tag, base64). Lives at `%APPDATA%\BarangayMS\bootstrap.json`, per ADR-0005, so it is captured by every Backup Snapshot (Phase 3's `infra_backup`) without new backup plumbing. This document does not define the snapshot mechanism itself — only that `bootstrap.json`'s location satisfies the "travels with the backup" requirement Phase 3 will build on.
...
- **Gate 1 (domain invariant tests + reviewed port contracts):**
  ...
  - Review of all four outbound port signatures (§8) for identity, ordering (`find_current`), atomicity (`save` as pure insert), and error-classification stability.
- **Gate 2 (compiling, intentionally failing use-case tests with local fakes):**
  ...
  - `GetActiveLicenseUseCase`, `EstablishEncryptionCredentialUseCase`, `RecoverDatabaseEncryptionKeyUseCase`, `GetMachineHardwareIdUseCase` — same pattern, one fake per port, no real I/O (`testing.md`'s "test-local outbound fakes perform no network, filesystem, database, or process I/O").
...
6. (Future tickets, HADR-0007 gates 4–5) `infra_hardware_id`, `infra_licensing`, `infra_credentials`, `infra_persistence`'s `LicenseGrantRepository`, and `src-tauri`'s `SystemClock` + composition + Tauri commands.
...
- [ ] All four outbound ports (§8.1–§8.4) plus `Clock` (§8.5) are declared `async` (except `Clock`) and object-safe behind `Arc<dyn Trait + Send + Sync>`.
...
- [ ] Every PRD §9.5/§9.6 requirement is either covered by a named use case/port in this document or explicitly deferred with a reason (§5); none are silently dropped.

## 15. Open Questions

- **Recovery Code and `bootstrap.json` loss before the first Backup Snapshot** (ADR-0005's own Neutral/Risks item): the earliest-window gap where `bootstrap.json` is lost before any snapshot captures it is not closed by this document. A first-run safeguard (forced immediate backup, or an explicit warning shown alongside the displayed Recovery Code) is left to the Phase 3 Backup & Recovery technical design.
- **Control Number Format token grammar** (PRD §14) — inherited, unresolved; Phase 2's problem.
- **Template Variable enumeration** (PRD §14) — inherited, unresolved; Phase 3's problem.
- **`infra_backup`'s exact interface to `bootstrap.json`** — this document fixes only that the file lives where snapshots naturally capture it (§10); how `infra_backup` enumerates/copies the file set is Phase 3 design, not fixed here.
- **MSVC toolchain path** (§4.3.1) — whether to invest in the Perl CPAN modules needed for the vendored-OpenSSL/MSVC build is left open; `x86_64-pc-windows-gnu` is the binding target either way, so this only affects future toolchain flexibility, not this document's acceptance criteria.

...
- **Control Number Format token grammar:** ~~Exact syntax and tokens for configurable Control Number Formats (e.g., `YYYY-NNNN` for year + zero-padded sequence, or `TYPE-YYYY-NN`, or other pattern language) is TDD-level detail.~~ **Resolved by [ADR-0007](../../adr/implemented/architecture/2026-08-18-0007-define-control-number-format-token-grammar.md)**: brace-delimited `{YYYY}` + `{N…}` tokens, literal prefix text, per-(Document Type × Philippine Standard Time calendar year) sequence scope, minimum-width padding, and an integer counter + frozen rendered string. The four seeded types ship with `BC-`/`COI-`/`COR-`/`CGMC-{YYYY}-{NNNNN}` defaults.
- **Template Variable availability:** ~~Complete enumeration of available Template Variables for Certificate Templates (currently outlined in [9.11](#911-certificate-template--layout-customization)) must be finalized during TDD based on data model feasibility.~~ **Resolved by [ADR-0008](../../adr/implemented/architecture/2026-08-18-0008-define-template-variable-enumeration.md)**: dotted `{{source.field}}` syntax, a closed 32-variable developer-seeded catalog (29 text + 3 image) across Resident/Household/Certificate/Barangay Official/Barangay Identity, standard `captain`/`secretary` position keys, empty-string on missing data, and computed values (age, assembled names) frozen at issuance.
